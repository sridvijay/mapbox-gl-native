...
void Painter::renderSymbol(PaintParameters& parameters,
    ...
    if (bucket.hasIconData()) {
        ...
        activeSpriteAtlas->bind(sdf || state.isScaling() || state.isRotating() || iconScaled || iconTransformed, context, 0);

        if (sdf) {
            renderSDF(bucket,
                      tile,
                      1.0f,
                      {{ activeSpriteAtlas->getWidth() / 4.0f, activeSpriteAtlas->getHeight() / 4.0f }},
            ...
            iconShader.u_texsize = {{ activeSpriteAtlas->getWidth() / 4.0f, activeSpriteAtlas->getHeight() / 4.0f }};
    ...
    if (bucket.hasTextData()) {
        ...
        renderSDF(bucket,
                  tile,
                  24.0f,
                  {{ glyphAtlas->width / 4.0f, glyphAtlas->height / 4.0f }},
